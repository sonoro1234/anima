...
void main()
{
	...
	vec3 collab;
	if (mode) 
		collab = XYZ2LAB(RGB2XYZ(color),D65);
	else
		collab = XYZ2LAB(RGB2XYZ(sRGB2RGB(color)),D65);
	...
	if (mode)
		...
		color = RGB2sRGB(XYZ2RGB(LAB2XYZ(collab,D65)));
	...
	{"bypass",false,guitypes.toggle}
		...
		for i=1,3 do
			if i~=curr_curve then
				...
				--ig.PushItemFlag(imgui.ImGuiItemFlags_Disabled, true);
