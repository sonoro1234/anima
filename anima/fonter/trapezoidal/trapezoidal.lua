...
local function TakeSnap(item)
	table.insert(Snaps,deepcopy(item))
end
local function CompareSnaps(item)
	for i=1,#Snaps do
		local eq = tbl_compare(Snaps[i],item)
...
function CG.edges_monotone_tesselator(P, contours,wrule,alg1)
	...
	local function StatusPointAdd(i)--,tipo)
		...
		elseif tipo=="end" then
			...
			local sorted = Status.sorted
			table.remove(sorted,hiorder)--loworder)
			table.remove(sorted,loworder)
	...
	local prioL,pointmin
	local function getPrioL(a,b)
		local preva,prevb = E.edges[a].prev,E.edges[b].prev
		local priomin = math.huge
		local pointmin
		for i,v in ipairs{a,b,preva,prevb} do
			local pri = getPrio(v)
	...
	local function SearchStatus(ii)
		local added_lines = {}
		...
		for i=1,#added_lines do
			...
			if sameface then
				...
				ListPrio(prioL,getPrio(l[1]),SearchStatus)
				...
				-- printD("--after l[2]",E:strEdge(l[2]))
				-- printD("--after bnew",E:strEdge(bnew))
				ListPrio(prioL,getPrio(l[1]),SearchStatus)
				 -- ListPrio(1,getPrio(l[1]),SearchStatus)
				--error"nnnnnnnnnnnnnnnnnnnnnnnnnnn"
				--E:faces_print()
		...
		INADDLINES = false
			...
			StatusPointAdd(ii)
			printStatus("after StatusPointAdd",ii,"prio",getPrio(ii))

			--[[
			if not INADDLINES then
				printD("taking snap",ii,"prio",getPrio(ii))

			else
				if not CompareSnapsP(Status, getPrio(ii),ii) then
					print("--FAIL compare in point",ii,"prio",getPrio(ii))
					...
					print("--GOOD compare in point",ii,"prio",getPrio(ii))
					--os.exit()
				end
			end
			--]]
